...
def save_report(report_data: dict[str, Any], db_path: str | Path | None = None) -> int:
    ...
    insert_sql = """
        INSERT INTO reports (
            dataset_signature, dataset_name, created_at,
...
def list_reports(
    ...
    """Return reports newest first, optionally filtered by dataset signature."""
    init_db(db_path)
    try:
        with _database(db_path) as connection:
            if dataset_signature is None:
                ...
                cursor = connection.execute(
                    "SELECT * FROM reports WHERE dataset_signature = ? "
...
def get_report(
    ...
    """Return one report by id, or None when it does not exist."""
    init_db(db_path)
    try:
        with _database(db_path) as connection:
            cursor = connection.execute("SELECT * FROM reports WHERE id = ?", (report_id,))
            row = cursor.fetchone()
